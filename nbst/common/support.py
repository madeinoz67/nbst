...
from nbst.common.logging import get_logger
...
def normalize_mac_address(mac_address=None):
    ...
    if ":" not in mac_address:
        mac_address = ":".join(
            mac_address[i : i + 2] for i in range(0, len(mac_address), 2)  # noqa: E203
        )
...
def ip_valid_to_add_to_netbox(ip, permitted_subnets, interface_name=None):
    ...
    permitted_subnets:
        list of permitted subnets where each subnet/prefix is an instance of
        IP4Network or IP6Network
    interface_name: str
        name of the interface this IP shall be added. Important for meaningful
        log messages
...
def perform_ptr_lookups(ips, dns_servers=None):
    ...
    Returns
    -------
    dict: of {"ip": "hostname"} for requested ips, hostname will be None if no
        hostname returned
    """
    ...
    if dns_servers is not None:
        if isinstance(dns_servers, list):
            log.debug2(
                "using provided DNS servers to perform lookup: %s"
                % ", ".join(dns_servers)
            )
...
async def reverse_lookup(resolver, ip):
    ...
    Returns
    -------
    dict: of {"ip": "hostname"} for requested ip, hostname will be None if no
     hostname returned
    """
